...
public class MyViewGroup extends ViewGroup {
	...
	private boolean isleft = true;//菜单是否从左边出
	 ...
	 * int b) { if (changed) { menu_view = getChildAt(0);// 获取滑动菜单的view
	 * content_view = getChildAt(1);// 获得主页view
	 * 
	 * // 相当于fill_parent content_view.measure(0, 0); content_view.layout(0, 0,
	 * getWidth(), getHeight()); // content_view.layout(0, 0,240, 320); } }
	 */
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if(isleft)
			...
			menu_view.layout(getWidth(), 0, getWidth() - 2 * distance, getHeight());
		content_view.layout(0, 0, getWidth(), getHeight());

		/*
		 * if(changed){ System.out.println("changed"); menu_view.measure(0,
		 * distance); menu_view.layout(-distance, 0, 0, getHeight());
		 * content_view.layout(0, 0, getWidth(), getHeight()); }
		 */
		// invalidate();
